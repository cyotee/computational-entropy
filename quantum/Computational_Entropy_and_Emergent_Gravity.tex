...
% (This embedded copy should eventually be replaced with \input{...} or equivalent)
...
\subsubsection{Bianconi (2025) -- Gravity from Relative Entropy}
Bianconi~\cite{Bianconi2025} derives modified Einstein equations from an entropic action: the quantum relative entropy between the spacetime metric (treated as a local quantum operator) and a matter-induced metric built from topological (Dirac--K\"ahler) bosonic fields and curvature. An auxiliary G-field yields a dressed Einstein--Hilbert action with an emergent small positive cosmological constant depending only on that field; the low-coupling limit recovers Einstein gravity with vanishing \(\Lambda\). This continuum construction lies in the same information-theoretic lineage as the present work (geometry co-determined with matter/information structure) but takes relative entropy of metrics-as-operators as primary, whereas we take computational entropy \(S_c = S(\Phi_g(\rho))\) of a channel output and a load-dependent master equation. The G-field, emergent \(\Lambda_G\), and discrete-network precursor of Bianconi are natural continuum targets for eventual variational or continuum-limit formulations of \(\Phi_g\) and \(L\).

As a constructive intermediate step---not a derivation of the continuum field equations---we formulate an \emph{Action--Channel Duality} in the Euclidean warm-up of GfE~\cite{Bianconi2025BeyondHolography}. There the second metric induced by a scalar field is \(G = 1 + \alpha_G |\nabla\phi|^2\), and the GfE warm-up gradient flow coincides with Perona--Malik anisotropic diffusion. Interpreting that flow as an observation channel reconstructing a hidden structure \(\phi_\star\) from noisy data, computational entropy is measured by residual reconstruction error (and edge-location uncertainty), while a split load \(L_E+L_S\) tracks induction intensity \(\mathbb{E}[G-1]\) and reparameterizes reconstructor time analogously to \(d\tau = dt/(1+\alpha L)\). On a one-dimensional lattice joint toy this dual passes a six-criterion scorecard (structure-preserving residual reduction versus isotropic heat, edge retention, ramp stability, load--induction correlation, and load-gated slowing). The shared Stage-2 object is the induced metric \(G\); Stage-3 GfE/Perona--Malik supplies the reconstructor; Stage-1 supplies \(H_c\) and the clock. Extending the dual from this Euclidean warm-up to Lorentzian GfE and to the master equation remains open.
...
As a complementary, fully explicit discrete validation of the information-theoretic layer---rather than of curved spacetime itself---we implement a Euclidean joint toy in which Perona--Malik / GfE warm-up dynamics act as a reconstructor channel on noisy one- and two-dimensional signals, with load-gated time reparameterization. The one-dimensional suite satisfies a six-criterion Action--Channel Duality scorecard (residual reduction versus heat, edge retention, ramp stability, load--induction tracking, and clock slowing). This supports simulability of the channel/load bookkeeping independently of a full continuum gravity simulation.
...
\bibitem{Bianconi2025} G. Bianconi, ``Gravity from entropy,'' Phys. Rev. D \textbf{111}, 066001 (2025); arXiv:2408.14391.
\bibitem{Bianconi2025BeyondHolography} G. Bianconi, ``Beyond holography: the entropic quantum gravity foundations of anisotropic diffusion,'' arXiv:2503.14048 (2025).
