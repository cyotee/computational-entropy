...
1. **Degeneracy with effective stress-energy.** Irreversible entropy production is usually accompanied by heat flux, and heat flux *is* in \(T_{\mu\nu}\) — which already gravitates in GR. Unless a regime exists where \(dS_c/d\tau\) is decoupled from any energy/heat flux, term (2) may be reabsorbable into an effective source and produce **no independent signature**. No such decoupled regime has been constructed.
2. **Calibration may suppress it.** \(\gamma\) is "fixed by matching to the Newtonian weak field and Bekenstein saturation." No calculation yet shows the matched \(\gamma\) leaves an observable residual in any accessible regime; it could be negligibly small.
