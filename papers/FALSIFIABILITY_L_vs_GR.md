...
**Update (2026-07-31) — R4a done, and sign-tested.** Route 1 (load-clock) is closed by the no-go. Route 2 (promotion into the field equations) was run ([../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md)): it yields a **consistent, GR-reducing candidate hypothesis** — an interacting dark sector giving **phantom** dark energy \(w_0=-1-0.143\,\xi\le-1\). Then two sharpening steps: (i) the **sign is derived** from the framework's own locked reading (load = entropy flux, not stockpile) + the second law ⇒ phantom — the data-favored quintessence sign is the framework's *rejected* anti-pattern, so it **cannot be flipped to fit data** (`r4a_sign_derivation.py`); (ii) the **w(z) test** finds current DESI hints prefer \(w_0>-1\) (quintessence), so the derived phantom prediction is **disfavored** (`r4a_wz_survival.py`). Then an **equation-of-state test** (`r4a_eos_test.py`): the dissipation end that gave phantom is **thermal** \((w\ge0)\) and cannot be dark energy — so **phantom is retracted** (EoS-inconsistent); only a \(w\approx-1\) reservoir could be dark energy. Then **Path 1** (`r4a_reservoir_eos.py`, data-independent) tested whether the framework *forces* that \(w\approx-1\) — and it does **not**: the load ties the scalar's *kinetic* term to the active flux \(\lvert dS_c/d\tau\rvert\), so the framework's emphasized high-flux regime is \(w\to+1\) (stiff, not dark energy); \(w\approx-1\) holds only in the **idle** regime (the de-emphasized stockpile), and the natural dynamics lean **freezing** \((w_a>0)\), the *wrong* side of the DESI hint (the earlier thawing claim is withdrawn). **Status: reformulation as a *clock*; under *promotion*, phantom is EoS-retracted and the reservoir/quintessence route is *not grounded* by the framework — it needs an ungrounded posit and its dynamics point the wrong way.** So neither promotion branch yields a clean, framework-grounded, data-consistent departure. The reformulation verdict is robust; a genuine departure would require *new field content* the framework does not supply.
