...
| **R4a promotion — departure as a field-equation modification** | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md) (`simulations/gravity-toy/r4a_frw_promotion.py`): promoting entropy production into Einstein's eqs is Bianchi-consistent, reduces to \(\Lambda\)CDM (\(\xi\to0\)), departs as **phantom interacting dark energy** \(w_0=-1-0.143\xi\), bounded \(\xi<0.35\). **Candidate hypothesis.** **Decisive open item:** does the theory fix the coupling \(\xi\) away from 0? (= the load-constant calibration). |
