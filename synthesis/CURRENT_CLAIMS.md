...
| C18 | **R4a promotion = candidate whose DERIVED sign is disfavored:** promotion is Bianchi-consistent, reduces to \(\Lambda\)CDM (\(\xi\to0\)), departs as **phantom** DE \(w_0=-1-0.143\xi\le-1\). **EoS test (data-independent) is decisive:** the dissipation/phantom end is **thermal** \((w\ge0)\) and **cannot be dark energy** (the phantom \(w_S=-1\) was EoS-inconsistent); only the **reservoir** end \((w\approx-1)\) is a viable dark sector, so requiring \(w\approx-1\) **forces** it ⇒ **thawing quintessence** \((w_0>-1,w_a<0)\), the direction DESI hints favor. Legitimate (EoS-motivated revision, not a data fit). **Caveats:** revises the locked reading; reservoir-as-scalar is a modeling choice; **magnitude still free** (dark-energy value not explained) | **derived model**; phantom retracted (EoS-inconsistent); EoS selects thawing quintessence; magnitude open | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md) §4b–4d |
