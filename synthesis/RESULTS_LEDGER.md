...
| O-5 | Central conjecture: departure regime **or** no-go. **Load-clock: NO-GO** (load is a diagnostic co-moving with the Path-J/M metric; \(\alpha L=-\Phi/c^2\) is \(T_{\mu\nu}\)-fixed while the entropy term varies at fixed \(T_{\mu\nu}\); [load-dim](../emergent-gravity/load-dimensional-analysis.md) · `regime_*`). **R4a promotion: CANDIDATE HYPOTHESIS** — promoting entropy production into the field equations is Bianchi-consistent, reduces to \(\Lambda\)CDM (\(\xi\to0\)), departs as **phantom interacting dark energy** \(w_0=-1-0.143\xi\le-1\) ([r4a](../emergent-gravity/r4a-promotion.md) · `r4a_frw_promotion.py`). **w(z) test** (`r4a_wz_survival.py`): current DESI hints prefer \(w_0>-1\) (quintessence side), so the natural Landauer-sign (phantom) prediction is **disfavored** (survives only at \(\xi\to0=\Lambda\)CDM); opposite sign would fit but reinterprets the physics. Data unsettled. | clock closed; promotion = candidate, natural sign disfavored by current data | Phase 2; regime program; PRD load rebuild; R4a; w(z) test |
