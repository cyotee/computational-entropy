...
| O-5 | Central conjecture: departure regime **or** no-go. **Load-clock: NO-GO** (load is a diagnostic co-moving with the Path-J/M metric; \(\alpha L=-\Phi/c^2\) is \(T_{\mu\nu}\)-fixed while the entropy term varies at fixed \(T_{\mu\nu}\); [load-dim](../emergent-gravity/load-dimensional-analysis.md) · `regime_*`). **R4a promotion: CANDIDATE, but the DERIVED sign is disfavored.** Promotion is Bianchi-consistent and reduces to \(\Lambda\)CDM (\(\xi\to0\)); it departs as **phantom** interacting dark energy \(w_0=-1-0.143\xi\le-1\) ([r4a](../emergent-gravity/r4a-promotion.md) · `r4a_frw_promotion.py`). **Sign** (`r4a_sign_derivation.py`, `r4a_sign_analysis.py`): *not* airtight (\(\lvert dS_c/d\tau\rvert\) is sign-neutral; load is output-based) but **phantom robustly favored** — every identification that tracks entropy production (loss flux, output-entropy rise, structure accumulating) ⇒ phantom; the only quintessence route is the free-energy **reservoir** reading = the framework's *rejected* stockpile anti-pattern. **w(z) test** (`r4a_wz_survival.py`): DESI hints prefer \(w_0>-1\) ⇒ phantom **disfavored** (survives only at \(\xi\to0=\Lambda\)CDM); quintessence rescue needs a **new independent principle** (open, must not fit data). Data unsettled. | clock closed; promotion = candidate; phantom robustly favored but disfavored by data; quintessence = open theory problem | Phase 2; regime; PRD; R4a; w(z); sign derivation + analysis |
