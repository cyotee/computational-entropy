...
| O-5 | Central conjecture: departure regime **or** no-go. **Load-clock: NO-GO** (load is a diagnostic co-moving with the Path-J/M metric; \(\alpha L=-\Phi/c^2\) is \(T_{\mu\nu}\)-fixed while the entropy term varies at fixed \(T_{\mu\nu}\); [load-dim](../emergent-gravity/load-dimensional-analysis.md) · `regime_*`). **R4a promotion: CANDIDATE, but the DERIVED sign is disfavored.** Promotion is Bianchi-consistent and reduces to \(\Lambda\)CDM (\(\xi\to0\)); it departs as **phantom** interacting dark energy \(w_0=-1-0.143\xi\le-1\) ([r4a](../emergent-gravity/r4a-promotion.md) · `r4a_frw_promotion.py`). **Sign DERIVED** (`r4a_sign_derivation.py`): the framework's locked reading (load = entropy **flux**, not stockpile) + 2nd law force \(Q\ge0\) ⇒ **phantom** — the quintessence sign requires the framework's *rejected* stockpile anti-pattern. **w(z) test** (`r4a_wz_survival.py`): current DESI hints prefer \(w_0>-1\) ⇒ the derived phantom prediction is **disfavored** (survives only at \(\xi\to0=\Lambda\)CDM); the quintessence rescue is **closed** within the theory. Data unsettled. | clock closed; promotion = candidate whose *derived* sign is disfavored by current (unsettled) data | Phase 2; regime; PRD; R4a; w(z); sign derivation |
