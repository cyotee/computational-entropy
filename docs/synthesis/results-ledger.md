...
| O-5 | Central conjecture: departure regime **or** no-go. **Load-clock: NO-GO** (load is a diagnostic co-moving with the Path-J/M metric; \(\alpha L=-\Phi/c^2\) is \(T_{\mu\nu}\)-fixed while the entropy term varies at fixed \(T_{\mu\nu}\); [load-dim](../emergent-gravity/load-dimensional-analysis.md) · `regime_*`). **R4a promotion: CANDIDATE HYPOTHESIS** — promoting entropy production into the field equations is Bianchi-consistent, reduces to \(\Lambda\)CDM (\(\xi\to0\)), and departs as **phantom interacting dark energy** \(w_0=-1-0.143\xi\), bounded by data \(\xi<0.35\) ([r4a](../emergent-gravity/r4a-promotion.md) · `r4a_frw_promotion.py`). | clock closed; promotion = candidate (coupling \(\xi\) open) | Phase 2; regime program; PRD load rebuild; R4a |
