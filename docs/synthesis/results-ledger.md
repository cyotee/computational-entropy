...
| O-5 | Central conjecture: departure regime **or** no-go. **Load-clock: NO-GO** (load is a diagnostic co-moving with the Path-J/M metric; \(\alpha L=-\Phi/c^2\) is \(T_{\mu\nu}\)-fixed while the entropy term varies at fixed \(T_{\mu\nu}\); [load-dim](../emergent-gravity/load-dimensional-analysis.md) · `regime_*`). **R4a promotion: CANDIDATE, but the DERIVED sign is disfavored.** Promotion is Bianchi-consistent and reduces to \(\Lambda\)CDM (\(\xi\to0\)); it departs as **phantom** interacting dark energy \(w_0=-1-0.143\xi\le-1\) ([r4a](../emergent-gravity/r4a-promotion.md) · `r4a_frw_promotion.py`). **Sign** (`r4a_sign_*`): entropy-arrow readings ⇒ phantom, but the **EoS test** (`r4a_eos_test.py`, data-independent) is decisive: the dissipation/phantom end is **thermal** \((w\ge0)\) and **cannot be dark energy** — the phantom branch used an EoS-inconsistent \(w_S=-1\). Only the **reservoir** end \((w\approx-1)\) is a viable dark sector ⇒ requiring \(w\approx-1\) **forces** it ⇒ **thawing quintessence** \((w_0>-1,w_a<0)\), the direction current DESI hints favor (consistency, not the test). **Legitimate** (EoS-motivated revision, not a data fit). Caveats: revises the locked reading; reservoir-as-scalar is a modeling choice; **magnitude free** (no dark-energy value explained). | clock closed; promotion candidate: phantom **retracted** (EoS-inconsistent); EoS **selects thawing quintessence** consistent with data direction; magnitude open | Phase 2; regime; PRD; R4a; w(z); sign; EoS test |
