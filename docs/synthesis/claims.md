...
| C18 | **R4a promotion = candidate whose DERIVED sign is disfavored:** promotion is Bianchi-consistent, reduces to \(\Lambda\)CDM (\(\xi\to0\)), departs as **phantom** DE \(w_0=-1-0.143\xi\le-1\). The sign is **not airtight** (\(\lvert dS_c/d\tau\rvert\) sign-neutral) but **phantom robustly favored**: every identification of \(\rho_S\) tracking entropy production ⇒ phantom; only the free-energy **reservoir** reading ⇒ quintessence = the *rejected* stockpile anti-pattern. **w(z):** DESI hints prefer \(w_0>-1\) ⇒ phantom **disfavored**; a quintessence version needs a **new independent principle** (open; must not fit data). Data unsettled | **derived model**; phantom favored but disfavored by data; quintessence = open problem | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md) §4b–4c |
