...
| C17 | **Load is a diagnostic, not an independent clock (no-go):** the calibration \(\alpha\beta=4\pi G/c^4\) is dimensionally invalid; the correct \(\alpha L=-\Phi/c^2\) is \(T_{\mu\nu}\)-fixed, yet the entropy term varies at fixed \(T_{\mu\nu}\) (dephasing) ⇒ the load carries no independent gravity-consistent clock effect and co-moves with the Path-J/M metric ⇒ **reformulation structurally** | **structural no-go** | [../emergent-gravity/load-dimensional-analysis.md](../emergent-gravity/load-dimensional-analysis.md) |
| C18 | **R4a promotion = candidate whose DERIVED sign is disfavored:** promotion is Bianchi-consistent, reduces to \(\Lambda\)CDM (\(\xi\to0\)), departs as **phantom** DE \(w_0=-1-0.143\xi\le-1\). **EoS + Path 1:** the dissipation/phantom end is **thermal** \((w\ge0)\) ⇒ **phantom retracted** (EoS-inconsistent). Only a \(w\approx-1\) reservoir is viable, but the framework does **not force** it (Path 1): the load ties the scalar's *kinetic* term to the active flux, so the emphasized high-flux regime is \(w\to+1\) (not DE); \(w\approx-1\) holds only in the **idle** regime (de-emphasized stockpile), and dynamics lean **freezing** \((w_a>0)\) — wrong side of the DESI hint (thawing retracted). **Net:** no clean framework-grounded, data-consistent dark-energy departure; reservoir-DE is an ungrounded posit | **derived model**; phantom retracted; reservoir-DE not grounded (idle-only, likely wrong-sign \(w_a\)); reformulation robust | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md) §4b–4e |
