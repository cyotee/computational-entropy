...
| C18 | **R4a promotion = candidate whose DERIVED sign is disfavored:** promotion is Bianchi-consistent, reduces to \(\Lambda\)CDM (\(\xi\to0\)), departs as **phantom** DE \(w_0=-1-0.143\xi\le-1\). The sign is **derived** from the framework's locked reading (load = entropy **flux**, not stockpile) + 2nd law ⇒ phantom; the quintessence sign is the *rejected* anti-pattern (cannot flip to fit data). **w(z):** DESI hints prefer \(w_0>-1\) ⇒ derived prediction **disfavored**; quintessence rescue **closed** within the theory. Data unsettled | **derived model**; derived sign disfavored by current (unsettled) data | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md) §4b–4c |
