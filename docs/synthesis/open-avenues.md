...
| **Entropy-production clock effect at fixed energy** (dephasing regime) | Regime program: [../papers/REGIME_PROGRAM_dSc_decoupling.md](../papers/REGIME_PROGRAM_dSc_decoupling.md). **Stage 1 DONE** (`regime_decoupling_witness.py`): dephasing decouples (\(\kappa\approx0\)) ⇒ departure *in principle*. **Stage 2 DONE** (`regime_gamma_calibration.py`): \(\alpha\gamma\) unfixed (dimensional gap); clocks bound \(\alpha\gamma\lesssim10^{-18}\)s; unmeasurable for natural \(\gamma\) ⇒ **reformulation in practice**. **Open (theory, not experiment):** dimensionally-consistent completion of the load constants |
