...
| **Entropy-production clock effect at fixed energy** (dephasing regime) | Regime program: [../papers/REGIME_PROGRAM_dSc_decoupling.md](../papers/REGIME_PROGRAM_dSc_decoupling.md). **Stage 1 DONE** (`simulations/regime/regime_decoupling_witness.py`): dephasing decouples (\(\kappa\approx0\)) ⇒ reabsorption horn ruled out; departure *in principle*. **Open:** Stage 2 \(\gamma\)-magnitude (calibration) → Stage 3 precision clocks |
