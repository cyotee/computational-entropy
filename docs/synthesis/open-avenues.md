...
| **Missing theorem (target)** | *There exists a scaling limit under which discrete densities converge to the continuum load terms*, with stated topology and rate—or a **precise obstruction**. Decay algebra: **proved** \(w=2\) (m11f), **general \(w\)/any gate** (m11g), **2D existence + strip transfer** (m11h); **open:** the **continuum embedding**. |
| **Suggested first step** | ~~lattice extensive~~/~~IDEM bound~~/~~1D algebra (m11f)~~/~~range-\(r\) (m11g)~~/~~general-\(w\) theorem (m11g)~~/~~2D strip (m11h)~~ **all done**. Next (final rung): the **continuum embedding** — scaling limit sending the discrete per-site entropy-production density to a continuum density (bridge to \(\gamma\lvert dS_c/d\tau\rvert\)), or a precise obstruction. |
