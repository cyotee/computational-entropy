...
| **Entropy-production clock effect at fixed energy** (dephasing regime) | Regime program: [../papers/REGIME_PROGRAM_dSc_decoupling.md](../papers/REGIME_PROGRAM_dSc_decoupling.md). **Stage 1** (`regime_decoupling_witness.py`): dephasing decouples ⇒ departure *in principle*. **Stage 2** (`regime_gamma_calibration.py`): \(\alpha\gamma\) unfixed; unmeasurable ⇒ reformulation in practice. **Load rebuild — NO-GO** ([../emergent-gravity/load-dimensional-analysis.md](../emergent-gravity/load-dimensional-analysis.md)): correct calibration \(\alpha L=-\Phi/c^2\) is \(T_{\mu\nu}\)-fixed, but the entropy term varies at fixed \(T_{\mu\nu}\) ⇒ load is a **diagnostic**, not a clock ⇒ **reformulation structurally**; \(\alpha\gamma\) not observable. **Closed** (a departure needs a different theory, R4a). |
