...
| **Entropy-production clock effect at fixed energy** (dephasing regime) | Regime program: [../papers/REGIME_PROGRAM_dSc_decoupling.md](../papers/REGIME_PROGRAM_dSc_decoupling.md). Stage 1 decouples; Stage 2 unmeasurable; **load rebuild NO-GO** ([load-dim](../emergent-gravity/load-dimensional-analysis.md)) ⇒ load is a diagnostic, not a clock ⇒ **reformulation structurally**. |
| **R4a promotion — departure as a field-equation modification** | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md): Bianchi-consistent, reduces to \(\Lambda\)CDM, departs as **phantom** DE \(w_0\le-1\). **Sign/EoS/Path 1** (`r4a_sign_*`, `r4a_eos_test.py`, `r4a_reservoir_eos.py`): dissipation/phantom end is thermal \((w\ge0)\) ⇒ **phantom retracted** (EoS-inconsistent). Only a \(w\approx-1\) reservoir is viable — but **Path 1** shows the framework does **not force** it: the load identifies the scalar *kinetic* term with the active flux, so the emphasized high-flux regime is \(w\to+1\) (not DE); \(w\approx-1\) holds only in the **idle** regime (de-emphasized stockpile), and dynamics lean **freezing** \((w_a>0)\) — wrong side of DESI. **Net:** no clean, framework-grounded, data-consistent dark-energy departure; reservoir-DE is an ungrounded posit. **Reformulation verdict robust.** Remaining long shots: a *different* field-content principle grounding \(w\approx-1\) in the active regime, or DESI DR2 shifting toward freezing/phantom. |
