...
| **R4a promotion — departure as a field-equation modification** | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md): Bianchi-consistent, reduces to \(\Lambda\)CDM, departs as **phantom** DE \(w_0\le-1\). **Sign** (`r4a_sign_derivation.py` + `r4a_sign_analysis.py`): not airtight, but **phantom robustly favored** — every entropy-arrow-tracking identification of \(\rho_S\) ⇒ phantom; only the free-energy **reservoir** (rejected stockpile) reading ⇒ quintessence. **w(z)**: DESI hints prefer \(w_0>-1\) ⇒ phantom **disfavored**. **Remaining life:** \(\xi\to0=\Lambda\)CDM, or DESI DR2 shifting to \(w_0<-1\). **Open theory problem (if pursued):** a *new, independently-motivated* principle identifying \(\rho_S\) with the reservoir end (the legitimate quintessence route) — must not be chosen to fit data. |
