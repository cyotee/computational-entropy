...
| **R4a promotion — departure as a field-equation modification** | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md): Bianchi-consistent, reduces to \(\Lambda\)CDM, departs as **phantom** DE \(w_0\le-1\). **Sign** (`r4a_sign_*`, `r4a_eos_test.py`): entropy-arrow readings give phantom, but the **equation of state** decides it data-independently — dissipation (phantom) is thermal \((w\ge0)\), **not** dark energy (the phantom \(w_S=-1\) was EoS-inconsistent); only the **reservoir** \((w\approx-1)\) is a viable dark sector ⇒ requiring \(w\approx-1\) **forces** it ⇒ **thawing quintessence** \((w_0>-1,w_a<0)\), consistent with the DESI direction. Legitimate (EoS-motivated, not a data fit). **Open (to make it a theory):** (a) justify reservoir-as-scalar storage from the framework independently; (b) fix the **magnitude** (still free); (c) extract an **independent** prediction beyond the thawing quadrant to test against data DESI did not fix. |
