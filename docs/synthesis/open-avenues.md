...
| **Have (2026-07-30 GENERALIZED)** | **Range-\(r\) generalization witnessed** (`simulations/classical/m11_decay_algebra_general.py`): the belief transfer becomes an **operator on the \(2^{w-1}\) window states** (\(w=r+1\)); it reproduces the enumerated density for AND/OR (\(w\le4\), exact, matches \(\sim10^{-6}\)) and threshold (\(w=3\), MC, \(\sim10^{-4}\)), recovering \(a=0.3007568\) at \(w=2\). **Finding:** the belief set is finite (⇒ exact algebra) **iff** the gate has a fully-recoverable (\([0,\dots,0]\)) decay branch (reset gates AND/OR); non-reset gates use MC. Numerical witness — general **theorem** still open. |
| **Must not claim** | \(L^{\mathrm{disc}}=L(\rho,g)\); IDEM constructs continuum metric \(G\); that the witnesses above are the limit theorem |
| **Missing theorem (target)** | *There exists a scaling limit* (graph → continuum, or many local maps → field \(\rho\)) *under which discrete work/export/capacity densities converge to the three continuum load terms*, with stated topology and rate—or a **precise obstruction** theorem. Decay algebra: **proved** \(w=2\); **witnessed** range-\(r\)/multi-gate; **open:** general-\(w\) theorem, 2D, continuum. |
| **Suggested first step** | ~~lattice with extensive export~~ **(done)**; ~~IDEM decay predicts density~~ **(done)**; ~~1D decay algebra~~ **(done + proved, m11f)**; ~~range-\(r\)/multi-gate generalization~~ **(done — transfer operator on \(2^{w-1}\) boundary)**. Next: (a) **general-\(w\) theorem** (prove the operator's stationary law gives \(a\); reset-gate finiteness is the lever); (b) **2D strip** transfer; (c) the **continuum embedding** (entropy-rate → continuum entropy-production density). |
