...
| **R4a promotion — departure as a field-equation modification** | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md): Bianchi-consistent, reduces to \(\Lambda\)CDM, departs as **phantom** DE \(w_0\le-1\). **Sign DERIVED** (`r4a_sign_derivation.py`): the locked reading (flux, not stockpile) + 2nd law force phantom; the quintessence sign is the framework's *rejected* anti-pattern (can't flip to fit data). **w(z) test**: DESI hints prefer \(w_0>-1\) ⇒ derived prediction **disfavored**; quintessence rescue **closed**. **Remaining life:** \(\xi\to0=\Lambda\)CDM (no new physics), or wait for DESI DR2 to see if data moves to \(w_0<-1\). A quintessence version = a *different* theory (revise the load reading for independent reasons). |
