...
| **Missing theorem (target)** | *There exists a scaling limit* (graph → continuum, or many local maps → field \(\rho\)) *under which discrete work/export/capacity densities converge to the three continuum load terms*, with stated topology and rate—or a **precise obstruction** theorem. Decay algebra: **proved** \(w=2\) (m11f) **and general \(w\)/any gate** (m11g); **open:** 2D strip, continuum embedding. |
| **Suggested first step** | ~~lattice with extensive export~~ **(done)**; ~~IDEM decay predicts density~~ **(done)**; ~~1D decay algebra~~ **(done + proved, m11f)**; ~~range-\(r\)/multi-gate generalization~~ **(done, witnessed)**; ~~general-\(w\) theorem~~ **(done + proved, m11g: existence + Blackwell + reset-regeneration)**. Next: (a) **2D strip** transfer (boundary = a cut); (b) the **continuum embedding** (entropy-rate density → continuum entropy-production density — the bridge to the load term). |
