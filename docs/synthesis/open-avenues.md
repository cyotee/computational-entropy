...
| **R4a promotion — departure as a field-equation modification** | [../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md) (`r4a_frw_promotion.py`): promotion is Bianchi-consistent, reduces to \(\Lambda\)CDM, departs as **phantom** interacting dark energy \(w_0=-1-0.143\xi\le-1\). **w(z) test** (`r4a_wz_survival.py`): current DESI hints prefer \(w_0>-1\) (quintessence) ⇒ the natural Landauer-sign prediction is **disfavored**; survives only at \(\xi\to0\). **Next (if pursued):** wait for DESI DR2, or study the **opposite-sign (quintessence) branch** — do **not** derive the coupling for the disfavored phantom branch yet. |
