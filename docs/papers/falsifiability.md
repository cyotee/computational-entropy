...
**Update (2026-07-31) — R4a done, and sign-tested.** Route 1 (load-clock) is closed by the no-go. Route 2 (promotion into the field equations) was run ([../emergent-gravity/r4a-promotion.md](../emergent-gravity/r4a-promotion.md)): it yields a **consistent, GR-reducing candidate hypothesis** — an interacting dark sector giving **phantom** dark energy \(w_0=-1-0.143\,\xi\le-1\). Then two sharpening steps: (i) the **sign is derived** from the framework's own locked reading (load = entropy flux, not stockpile) + the second law ⇒ phantom — the data-favored quintessence sign is the framework's *rejected* anti-pattern, so it **cannot be flipped to fit data** (`r4a_sign_derivation.py`); (ii) the **w(z) test** finds current DESI hints prefer \(w_0>-1\) (quintessence), so the derived phantom prediction is **disfavored** (`r4a_wz_survival.py`). Then a decisive **equation-of-state test** (`r4a_eos_test.py`, data-independent): the dissipation end that gave phantom is **thermal** \((w\ge0)\) and **cannot be dark energy** — the phantom \(w_S=-1\) was EoS-inconsistent, so **phantom is retracted**. Only the **reservoir** end \((w\approx-1)\) is a viable dark sector, so requiring \(w\approx-1\) **forces** the reservoir reading ⇒ **thawing quintessence** \((w_0>-1,w_a<0)\) — the *direction* current DESI hints favor, reached by physics (EoS), **not** by fitting. **Status: reformulation as a *clock*; under *promotion*, an EoS-selected candidate for thawing quintessence — a legitimate, data-consistent direction — but with a free magnitude (dark-energy value unexplained), a revised load reading, and a reservoir-as-scalar modeling assumption still to be justified.** To become a theory it needs the magnitude fixed and an independent prediction beyond the thawing quadrant.
